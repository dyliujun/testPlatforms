...
def saveUnStatisticsData(request):
    ...
    apiStatisticsData = ApiStatisticsNew.objects.create(
...
def saveUnDoStatisticsData(request):
    ...
    print()
    if len(ApiStatisticsNew.objects.filter(path=path).order_by("created")) >0:
        print("接口已统计，修改信息")
        apiStatisticsData = ApiStatisticsNew.objects.filter(path=path).update(
